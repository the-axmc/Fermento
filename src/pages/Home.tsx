...
// Client generated by `stellar scaffold watch --build-clients`
// Adjust the relative path if your scaffold places it elsewhere.
...
/** Type guards & helpers to safely read dynamic shapes */
...
/**
 * Try to extract a wallet public key / address from different library shapes:
 * - { publicKey: string }
 * - { address: string }
 * - { accountId: string }
...
const initialSeed: FermentationProduct[] = [
  ...
  },
  {
    ...
    progress: 85,
    metadata: {
      ingredients: "Cabbage, Gochugaru, Salt, Garlic",
      temperature: "18°C",
      ph: "3.8",
...
// The scaffold client often exports functions directly, or under default.
// We accept either shape and keep everything typed as `unknown` return.
type NftContract = {
...
const Home: React.FC = () => {
  const wallet = useWallet(); // shape may vary by lib
  ...
  // Finalize product on-chain (compatible with scaffold clients)
  const transformProduct = async (productId: string) => {
    ...
    try {
      ...
      if (typeof c.mint === "function") {
        res = await c.mint({ to: walletPublicKey, token_id: target.id, uri });
      } else if (typeof c.mint_to === "function") {
        res = await c.mint_to({ to: walletPublicKey, token_id: target.id, uri });
      } else if (typeof c.hello === "function") {
        // some scaffold templates expose a simple demo method
          ...
          {isAdding && (
            <>
              ...
              <div style={{ display: "grid", gap: 12 }}>
                <div>
                  <label style={labelCol} htmlFor="name">
                    Product Name
                  </label>
                  <input
                    id="name"
                ...
                <div>
                  <label style={labelCol} htmlFor="type">
                    Type
                  </label>
                  <select
                    id="type"
                ...
                <div>
                  <label style={labelCol} htmlFor="ingredients">
                    Ingredients
                  </label>
                  <input
                    id="ingredients"
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                  <div>
                    <label style={labelCol} htmlFor="temperature">
                      Temperature
                    </label>
                    <input
                      id="temperature"
                      ...
                      onChange={(e) => setNewProduct((p) => ({ ...p, temperature: e.target.value }))}
                      placeholder="22°C"
                    />
                  </div>
                  <div>
                    <label style={labelCol} htmlFor="ph">
                      pH Level
                    </label>
                    <input
                      id="ph"
                      style={inputStyle}
                      value={newProduct.ph}
                      onChange={(e) => setNewProduct((p) => ({ ...p, ph: e.target.value }))}
                      placeholder="4.2"
                    />
                  </div>
                </div>
                ...
                <div>
                  <label style={labelCol} htmlFor="notes">
                    Notes
                  </label>
                  <textarea
                    id="notes"
                ...
                <button type="button" onClick={addProduct} style={primaryButtonStyle}>
                  Register Product
                </button>
              </div>
            </>
              ...
              <section key={product.id} style={cardStyle} aria-label={product.name}>
                <header style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}>
                  <div>
                    ...
                    <div style={smallNote}>
                      {product.type} • Started {product.startDate}
                    </div>
                  </div>
                  ...
                  <>
                    ...
                    <div style={{ display: "grid", gap: 16 }}>
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 12 }}>
                        <div>
                          <div style={labelCol}>Type</div>
                          <div>{product.type}</div>
                        </div>
                        <div>
                          <div style={labelCol}>Started</div>
                          <div>{product.startDate}</div>
                        </div>
                        <div>
                          <div style={labelCol}>Temperature</div>
                          <div>{product.metadata.temperature}</div>
                        </div>
                        <div>
                          <div style={labelCol}>pH Level</div>
                          <div>{product.metadata.ph}</div>
                        </div>
                      </div>

                      <div>
                        <div style={labelCol}>Ingredients</div>
                        <div style={{ fontSize: 14 }}>{product.metadata.ingredients}</div>
                      </div>
                      <div>
                        <div style={labelCol}>Notes</div>
                        <div style={{ fontSize: 14 }}>{product.metadata.notes}</div>
                      </div>
                      ...
                      <div>
                        ...
                        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                          <button
                            type="button"
                            style={buttonStyle}
                            onClick={() => updateProgress(product.id, product.progress + 10, "Progress update")}
                          >
                            +10%
                          </button>
                          <button
                            type="button"
                            style={buttonStyle}
                            onClick={() => updateProgress(product.id, product.progress + 25, "Significant progress")}
                          >
                            +25%
                          </button>
                          <button
                            type="button"
                            style={buttonStyle}
                            onClick={() => updateProgress(product.id, 100, "Fermentation complete")}
                          >
                            Complete
                          </button>
                        </div>
